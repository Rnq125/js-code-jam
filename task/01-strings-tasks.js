...
function removeFirstOccurrences(str, value) {
    ...
    str.substring((position + size), str.length);
}
...
function getRectangleString(width, height) {
  ...
  let main = '│' + ' '.repeat(width - 2) + '│\n';
  let footer = '└' + '─'.repeat(width - 2) + '┘\n';
  return header + main.repeat(height - 2) + footer;
}
 ...
 function encodeToRot13(str) {
   ...
   for (let i = 0; i < str.length; i++) {
     for (let j = 0; j < direct.length; j++) {
       if ( str.charAt(i) === direct.charAt(j) ) {
         result += reverse.charAt(j);
       }
     }
   }
   return result;
